class ClocksController < ApplicationController
  ...
  def clock_in
    ...
    if @clock.save
      ClockMailer.receipt_get_in(current_user, @clock).deliver_now unless current_user.email.nil?
  ...
  def clock_out
    ...
    if @clock.update(end_at: Time.now, active: false)
      ClockMailer.receipt_get_out(current_user, @clock).deliver_now unless current_user.email.nil?
